...
use crate::hardware::{
    AcquisitionModel, DeliveryModel, DeviceCapabilities, DeviceInfo, DeviceListing, GainModel,
...
fn worker_entry(
    ...
    let initialized = initialize(&mut *port);
    let (identity, settings) = match initialized {
        Ok(value) => value,
        Err(error) => {
...
fn initialize(port: &mut dyn SerialPort) -> anyhow::Result<(Identity, ScanSettings)> {
    ...
    let (settings, commands) = startup_settings(identity.model);
